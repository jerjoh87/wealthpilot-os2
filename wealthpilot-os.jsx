      ...
      <div className="card">
        ...
        {budgets.length === 0 ? (
          <div className="empty-state">
            <div className="icon">📭</div>
            <p className="text-sm">Create your first budget category.</p>
            <button className="btn btn-primary btn-sm" style={{marginTop:10}} onClick={()=>setOpen(true)}>+ Add Category</button>
          </div>
        ) : budgets.map(b => {
      ...
      <div className="grid-2">
        <div className="card">
          ...
          {unpaid.length === 0 ? (
            <div className="empty-state">
              <div className="icon">📭</div>
              <p className="text-sm">Add your first bill.</p>
              <button className="btn btn-primary btn-sm" style={{marginTop:10}} onClick={()=>setOpen(true)}>+ Add Bill</button>
            </div>
          ) : unpaid.map(b => (
          ...
          <div className="card settings-section">
            ...
            <div style={{marginTop:10}}>
              {(manualIncomeEntries||[]).length===0 ? (
                <div className="empty-state">
                  <div className="icon">💵</div>
                  <p className="text-sm">Add income to start your plan.</p>
                  <button className="btn btn-primary btn-sm" style={{marginTop:10}} onClick={()=>setIncomeFormOpen(true)}>Enter Income</button>
                </div>
              ) : (manualIncomeEntries||[]).map(i=><div key={i.id} className="integration-card" style={{marginBottom:6}}><div className="int-info"><div className="int-name">{i.source_name}</div><div className="int-status">{fmt(i.amount)} · {i.frequency} · Est monthly {fmt(incomeToMonthly(i))} · {i.payment_method}</div></div><div style={{display:'flex',gap:6}}><button className="btn btn-ghost btn-sm" onClick={()=>{setEditingIncomeId(i.id);setIncomeForm({ source_name:i.source_name||'', amount:String(i.amount||''), frequency:i.frequency||'Weekly', next_pay_date:i.next_pay_date||'', payment_method:i.payment_method||'Cash', notes:i.notes||'', monthly_estimate:String(i.monthly_estimate||'') });setIncomeFormOpen(true);}}>Edit</button><button className="btn btn-danger btn-sm" onClick={()=>setManualIncomeEntries((manualIncomeEntries||[]).filter(x=>x.id!==i.id))}>Delete</button></div></div>)}
            </div>
          </div>
          ...
          <div className="card settings-section">
            ...
            <div style={{marginTop:10}}>
              {(manualAccounts||[]).length===0 ? (
                <div className="empty-state">
                  <div className="icon">🏦</div>
                  <p className="text-sm">Connect a bank or add an account manually.</p>
                  <div style={{display:'flex',gap:8,justifyContent:'center',marginTop:10,flexWrap:'wrap'}}>
                    <button className="btn btn-primary btn-sm" onClick={handlePlaidConnect} disabled={totalConnectedAccounts >= MAX_ACCOUNTS}>Connect Bank</button>
                    <button className="btn btn-ghost btn-sm" onClick={()=>setAccountFormOpen(true)} disabled={totalConnectedAccounts >= MAX_ACCOUNTS}>Add Manual Account</button>
                  </div>
                </div>
              ) : (manualAccounts||[]).map(a=><div key={a.id} className="integration-card" style={{marginBottom:6,display:'grid',gap:8}}><div style={{display:'flex',justifyContent:'space-between',gap:8,alignItems:'center'}}><div className="int-info"><div className="int-name">{a.name}</div><div className="int-status">{a.type} · {fmt(a.balance)}</div></div><div style={{display:'flex',gap:6,flexWrap:'wrap',justifyContent:'flex-end'}}><button className="btn btn-ghost btn-sm" onClick={()=>{setIncomeTopUpAccountId(a.id);setIncomeTopUpInput('');}}>+ Income</button><button className="btn btn-ghost btn-sm" onClick={()=>{setBalanceFormAccountId(a.id);setBalanceInput(String(a.balance||0));}}>Update Balance</button><button className="btn btn-ghost btn-sm" onClick={()=>{setEditingAccountId(a.id);setAccountForm({ account_name:a.name||'', account_type:a.type||'Cash', starting_balance:String(a.balance||0), income_source_name:'', income_amount:'', income_frequency:'Weekly', payment_method:'Cash', next_pay_date:'', notes:'' });setAccountFormOpen(true);}}>Edit</button><button className="btn btn-danger btn-sm" onClick={()=>setManualAccounts((manualAccounts||[]).filter(x=>x.id!==a.id))}>Delete</button></div></div>{incomeTopUpAccountId===a.id && <div style={{display:'flex',gap:8}}><input className="form-input" placeholder="Income amount" value={incomeTopUpInput} onChange={e=>setIncomeTopUpInput(e.target.value)} /><button className="btn btn-primary btn-sm" onClick={()=>addIncomeToManualAccount(a.id)}>Apply</button><button className="btn btn-ghost btn-sm" onClick={()=>setIncomeTopUpAccountId(null)}>Cancel</button></div>}{balanceFormAccountId===a.id && <div style={{display:'flex',gap:8}}><input className="form-input" placeholder="New balance" value={balanceInput} onChange={e=>setBalanceInput(e.target.value)} /><button className="btn btn-primary btn-sm" onClick={()=>updateManualAccountBalance(a.id)}>Save</button><button className="btn btn-ghost btn-sm" onClick={()=>setBalanceFormAccountId(null)}>Cancel</button></div>}</div>)}
            </div>
          </div>
...
class AppErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error("WealthPilot UI crashed", error, errorInfo);
  }
