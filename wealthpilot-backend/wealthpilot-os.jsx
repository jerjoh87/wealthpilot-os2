            ...
            <div key={g.id} className="card" style={{borderLeft:`3px solid ${done?"var(--green)":color}`}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{marginTop:8,padding:"8px 10px",background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:10,fontSize:11,color:"var(--text2)"}}><div style={{display:"flex",gap:14,flexWrap:"wrap"}}><span>Status: <b style={{color:autoPlan.status==="completed"?"var(--green)":autoPlan.status==="behind"?"var(--red)":"var(--accent)",textTransform:"capitalize"}}>{autoPlan.status}</b></span><span>Amount remaining: <b style={{color:"var(--text)"}}>{fmt(autoPlan.amountRemaining)}</b></span><span>Weeks remaining: <b style={{color:"var(--text)"}}>{autoPlan.weeksRemaining ?? "—"}</b></span><span>Needed/week: <b style={{color:"var(--green)"}}>{fmt(autoPlan.amountNeededPerWeek)}</b></span>{autoPlan.amountNeededPerPayday !== null && <span>Needed/payday: <b style={{color:"var(--green)"}}>{fmt(autoPlan.amountNeededPerPayday)}</b></span>}</div></div>
                </div>
