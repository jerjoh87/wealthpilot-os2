...
export default function handler(_req: NextApiRequest, res: NextApiResponse<HealthPayload>) {
  ...
  if (!checks.gemini) warnings.push('GEMINI_API_KEY is missing')
  if (!checks.groq) warnings.push('GROQ_API_KEY is missing')
  if (!checks.openrouter) warnings.push('OPENROUTER_API_KEY is missing')
